...
while getopts 'avp:' OPTION; do
  case "$OPTION" in
    a)
	  ...
	  tsp youtube-dl --add-metadata -ic -f bestaudio/best -o '~/dl/%(upload_date)s-%(title)s.%(ext)s' $url
    ...
    v)
      FILENAME=$(youtube-dl --get-filename -o "%(title)s." $url)
	  tsp notify-send " Download started." "$FILENAME"
	  tsp youtube-dl --add-metadata -ic -o '~/dl/%(upload_date)s-%(title)s' $url
    ...
    p)
	  ...
	  tsp youtube-dl --add-metadata -ic -o '~/dl/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s' $url
	  tsp notify-send " Downloading complete." "$FILENAME"
      ;;
	*)
